...
async def photo(
    ...
    if frame is None:
        return Response(
            ...
            content="Failed to read frame from camera",
        )
...
async def hls_proxy(
    ...
    response = await session.get(
        f"{settings.mediamtx_hls_url}/{filename}?{query_params_string}"
    )
    ...
    return StreamingResponse(
        content=get_chunk(), status_code=status_code, headers=headers
    )
